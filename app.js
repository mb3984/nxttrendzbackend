...
const authenticateToken = (request, response, next) => {
  const authHeader = request.cookies.jwt_token; // Get token from cookies
  console.log("Received token:", authHeader); // Debugging line

  if (authHeader) {
    jwt.verify(authHeader, process.env.JWT_SECRET, (error, payload) => {
      if (error) {
        console.log("JWT verification error:", error);
        return response.status(401).send({ error: "Invalid JWT Token" });
      }
      request.userId = payload.userId; // Attach userId to request object
      console.log("Authenticated user ID:", request.userId); // Debugging line
      next();
    });
  } else {
    console.log("Authorization token missing");
    response.status(401).send({ error: "Authorization token missing" });
  }
};
...
app.post("/login", async (request, response) => {
  try {
    ...
    if (isPasswordMatched) {
      ...
      // Set token in cookies (optional, if you want to use cookies)
      ...
      return response.status(200).send({
        jwtToken: token, // Send token in response body
        userId: user._id,
      });
    } else {
      return response.status(401).send({ errorMsg: "Incorrect password" });
    }
  } catch (error) {
...
app.get(
  ...
  async (request, response) => {
    try {
      const collection = client.db("authentication").collection("users");
      const userId = request.userId; // Get userId from the authenticated token
      ...
      if (user) {
        ...
        response.status(200).send({
          username: user.name || "No name found",
          email: user.email || "No email found", // Include email in the response
          // You can add more user details here if needed
        });
        console.log({ username: user.name, email: user.email }, userId);
      } else {
